...
/// @notice Interface for LexDAO {memberAction} that adjusts member `shares` & `loot`.
...
contract LexDAO {
    ...
    bytes32 constant DOMAIN_TYPEHASH = keccak256('EIP712Domain(string name,uint chainId,address verifyingContract)'); /*EIP-712 typehash for DAO domain*/
    bytes32 constant DELEGATION_TYPEHASH = keccak256('Delegation(address delegatee,uint nonce,uint expiry)'); /*EIP-712 typehash for DAO delegation*/
    ...
    bytes32 constant VOTE_TYPEHASH = keccak256('Vote(uint proposalId,bool support)'); /*EIP-712 typehash for DAO proposal vote*/
    ...
    mapping(address => bool)   public extensions; /*maps contracts approved in 'whitelist'[3] proposals for {memberAction} that mint or burn `shares`*/
    
    event SummonComplete(bool lootPaused, bool sharesPaused, uint gracePeriod, uint minVotingPeriod, uint maxVotingPeriod, string name, string symbol, address[] guildTokens, address[] extensions, address[] summoners, uint96[] loot, uint96[] shares); /*emits after DAO summoning*/
    ...
    event Ragequit(address indexed member, address to, uint96 indexed lootToBurn, uint96 indexed sharesToBurn); /*emits when users burn DAO `shares` and/or `loot` for given `to` account*/
    event Approval(address indexed owner, address indexed spender, uint amount); /*emits when DAO `shares` are approved for pulls with erc20 accounting*/
    event Transfer(address indexed from, address indexed to, uint amount); /*emits when DAO `shares` are minted, burned or transferred with erc20 accounting*/
    event TransferLoot(address indexed from, address indexed to, uint96 amount); /*emits when DAO `loot` is minted, burned or transferred*/
    ...
    struct Checkpoint { /*DAO checkpoint for marking number of delegated votes*/
        uint32 fromTimeStamp; /*unix time for referencing voting balance*/
        uint96 votes; /*votes at given unix time*/
    }
 
    struct Member { /*DAO membership details*/
    ...
    struct Proposal { /*DAO proposal details*/
        ...
        address[] to; /*account(s) that receive(s) DAO state updates*/
        uint96[] value; /*value(s) associated with DAO state updates (also used to toggle)*/
        bytes[] data; /*raw data associated with DAO state updates (also used to toggle)*/
        string details; /*human-readable context for proposal*/
    }

    /// @notice Summon DAO with voting configuration & initial array of `members` accounts with `shares` & `loot` weights.
    /// @param _sharesPaused Sets transferability of DAO voting shares on initialization - if 'paused', `loot` will also be 'paused'.
    ...
    /// @param _extensions External contracts approved for {memberAction} that adjust `shares` & `loot`.
    ...
    constructor(
        ...
        unchecked {
            for (uint i; i < _extensions.length; i++) extensions[_extensions[i]] = true; /*update mapping of approved `extensions` in DAO*/
            ...
            for (uint i; i < _summoners.length; i++) {
                _mintLoot(_summoners[i], _loot[i]); /*mint DAO `loot` to `summoners`*/
                _mintShares(_summoners[i], _shares[i]); /*mint DAO `shares` to `summoners`*/ 
                _delegate(_summoners[i], _summoners[i]); /*delegate `summoners` voting weights to themselves - this saves a step before voting*/
                if (_summoners.length == 1) singleSummoner = true; /*flag if DAO summoned singly for speedy processing*/
            }
        }
        ...
        name = _name; /*initialize DAO `name` with erc20 accounting*/
        symbol = _symbol; /*initialize DAO `symbol` with erc20 accounting*/
        status = 1; /*initialize 'reentrancy guard' status*/
        
        emit SummonComplete(lootPaused, _sharesPaused, _gracePeriod, _minVotingPeriod, _maxVotingPeriod, _name, _symbol, _guildTokens, _extensions, _summoners, _loot, _shares); /*emit event reflecting DAO summoning completed*/
    }

    /// @notice Execute membership action to mint or burn `shares` and/or `loot` against whitelisted `extensions` in consideration of user & given amounts.
    ...
    function memberAction(
        ...
        require(extensions[extension],'!extension'); /*check `extension` is approved*/
        
        (lootOut, sharesOut) = IExtension(extension).memberAction{value: msg.value}(msg.sender, loot, shares); /*fetch 'reaction' per inputs*/
        
        if (mint) { /*execute `mint` actions*/
            if (lootOut != 0) _mintLoot(msg.sender, lootOut); /*add `loot` to user account & DAO total*/
            if (sharesOut != 0) _mintShares(msg.sender, sharesOut); /*add `shares` to user account & DAO total with erc20 accounting*/
        } else { /*otherwise, execute `burn` actions*/
            if (lootOut != 0) _burnLoot(msg.sender, lootOut); /*subtract `loot` from user account & DAO total*/
            if (sharesOut != 0) _burnShares(msg.sender, sharesOut); /*subtract `shares` from user account & DAO total with erc20 accounting*/
        }
    }
    ...
    PROPOSAL FUNCTIONS
    *****************/
    /// @notice Submit proposal to DAO `members` for approval within given voting period.
    ...
    function submitProposal(
        ...
        require(minVotingPeriod <= votingPeriod && votingPeriod <= maxVotingPeriod,'!votingPeriod'); /*check voting period is within DAO bounds*/
    ...
    /// @notice Sponsor proposal to DAO `members` for approval within voting period.
    ...
    function processWhitelistProposal(Proposal memory prop) private {
        unchecked {
            for (uint i; i < prop.to.length; i++) {
                if (prop.value[i] == 0 && prop.data[i].length == 0) { /*if `value` & `data` are null, approve `extensions`*/
                    extensions[prop.to[i]] = true; /*add account(s) to `extensions` extensions*/
                } else if (prop.value[i] == 0 && prop.data[i].length != 0) { /*if `value` is null & `data` is populated, remove `extensions`*/
                    extensions[prop.to[i]] = false; /*remove account(s) from `extensions` extensions*/
    ...
    *******************/
    ...
    /// @param lootToBurn DAO pure economic weight to burn.
    /// @param sharesToBurn DAO voting weight to burn.
    function ragequit(address to, uint96 lootToBurn, uint96 sharesToBurn) external nonReentrant {
        ...
        for (uint i; i < guildTokens.length; i++) {
            (,bytes memory balanceData) = guildTokens[i].staticcall(abi.encodeWithSelector(0x70a08231, address(this))); /*get DAO token balances - 'balanceOf(address)'*/
            uint balance = abi.decode(balanceData, (uint)); /*decode DAO token balances for calculation*/
        ...
        if (lootToBurn != 0) { /*gas optimization*/ 
            _burnLoot(msg.sender, lootToBurn); /*subtract `loot` from user account & DAO totals*/
        }
        
        if (sharesToBurn != 0) { /*gas optimization*/ 
            _burnShares(msg.sender, sharesToBurn);  /*subtract `shares` from user account & DAO totals with erc20 accounting*/
        }
        
        emit Ragequit(msg.sender, to, lootToBurn, sharesToBurn); /*event reflects claims made against DAO*/
    }
    ...
    ***************/
    ...
    /// @notice Returns array list of approved `guildTokens` in DAO for {ragequit}.
    /// @return tokens ERC-20s approved for {ragequit}.
    function getGuildTokens() external view returns (address[] memory tokens) {
        tokens = guildTokens;
    }
    
    /// @notice Returns `flags` for given DAO `proposal` describing type ('action'[0], 'member'[1], 'period'[2], 'whitelist'[3]).
    ...
    /// @return fee The `fee` amount of 'token' to be charged for the loan, on top of the returned principal - uniform in DAO.
    ...
    function maxFlashLoan(address token) external view returns (uint max) {
        (,bytes memory balanceData) = token.staticcall(abi.encodeWithSelector(0x70a08231, address(this))); /*get DAO token balance - 'balanceOf(address)'*/
        max = abi.decode(balanceData, (uint)); /*decode DAO token balance for calculation*/
    }

    /***************
    HELPER FUNCTIONS
    ***************/
    /// @notice Allows batched calls to DAO.
    ...
    /// @notice Returns confirmation for 'safe' ERC-721 (NFT) transfers to DAO.
    ...
    /// @notice Returns confirmation for 'safe' ERC-1155 transfers to DAO.
    ...
    /// @notice Returns confirmation for 'safe' batch ERC-1155 transfers to DAO.
    ...
    /// @notice Deposits ETH sent to DAO.
    receive() external payable {}

    /// @notice Delegates DAO voting weight.
    ...
    /// @notice Minting function for DAO `loot`.
    function _mintLoot(address to, uint96 loot) private {
        unchecked {
            if (totalSupply + loot <= type(uint96).max / 2) {
                ...
                totalLoot += loot; /*add to total DAO `loot`*/
    ...
    /// @notice Minting function for DAO `shares`.
    function _mintShares(address to, uint96 shares) private {
        unchecked {
            if (totalSupply + shares <= type(uint96).max / 2) {
                ...
                totalSupply += shares; /*add to total DAO `shares`*/
